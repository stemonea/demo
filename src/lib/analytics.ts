...
function aggregate(turns: AnalysedTurn[]): DebateStats {
  ...
  for (const turn of turns) {
    ...
    const byline = bylineOf(nodes)
    const speaker = turn.speaker || (byline ? textOf(byline).trim() : '') || 'UNKNOWN'
...
interface VisitOptions {
  turn: number
  speaker: string
  /** the turn's own `<person>NAME</person>:`, which says who is speaking and is not a mention */
  byline: TagNode | null
  onMention: (mention: Mention) => void
  onComponent: (inside: Mention[]) => void
}

function visit(nodes: TagNode[], inside: Grounding, options: VisitOptions) {
  for (const node of nodes) {
    if (node.type !== 'tag') continue

    const spec = getTagSpec(node.name)
    if (!spec) {
      visit(node.children, inside, options)
      continue
    }
    ...
    if (node !== options.byline && surface) {
      options.onMention({
        turn: options.turn,
...
/**
 * The `<person>NAME</person>:` a turn opens with, which says who is speaking
 * and is not something they said.
 *
 * Only that one: the first tag of the turn, with nothing but whitespace before
 * it and a colon straight after it. It used to be whichever person was named
 * first outside an argument, wherever that fell - so in a turn whose speaker
 * was not tagged, `MODERATOR: Welcome <person>Obama</person>`, the first person
 * actually mentioned was taken for the byline and never counted.
 */
function bylineOf(nodes: TagNode[]): Extract<TagNode, { type: 'tag' }> | null {
  for (let at = 0; at < nodes.length; at += 1) {
    ...
    if (node.type !== 'tag' || node.name !== 'person') return null
    const next = nodes[at + 1]
    return next?.type === 'text' && /^\s*:/.test(next.value) ? node : null
  }
  return null
}
