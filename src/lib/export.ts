...
function walk(nodes: TagNode[], context: Context, state: Extraction & { plain: string }) {
  for (const node of nodes) {
    ...
    /* the name this app knows the tag by, not the one it arrived under: the
       model writes `<organization>` and a correction of the same turn writes
       ORG, so the two exports of one annotation carry one label */
