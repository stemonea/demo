 ...
 * spans that half-overlap - is reported instead of repaired
...
import { canAdd, layerOf, newSpanId, outerFirst, tokenize, type Layer, type ManualSpan } from './manual'
...
function looksLikeConll(text: string): boolean {
  const line = text.split('\n').find((candidate) => candidate.trim() && !isComment(candidate))
  return Boolean(line && line.split('\t').length >= 2)
}

/**
 * A comment in a column file: `# token\targument\tentity`, `# sent_id = 3`.
 *
 * Not every line that starts with `#` is one. `#` is a token like any other,
 * and the export writes it as `#\tO\tO` - read as a comment, that row was
 * dropped and the `#` went missing from the text. A comment is the mark
 * followed by a space, or the mark alone; a token is followed by its columns.
 */
...
function fromInline(source: string): Draft {
  ...
  const walk = (nodes: TagNode[]) => {
    for (const node of nodes) {
      ...
      if (node.type === 'orphan') {
        /* a stray closer of the schema is a defect of the annotation; any other
           name in angle brackets is part of what was said, and stays in it */
        if (getTagSpec(node.name)) warnings.push(`Dropped a closing </${node.name}> that opened nowhere.`)
        else text += node.raw
        continue
      }

      /*
       * A mark of a name the schema does not have, never closed, is not an
       * annotation. Transcripts write stage directions that way - `<crosstalk>`,
       * `<laughter>` - and read as a tag it became an entity running to the end
       * of the turn, while the direction itself vanished from the text. It is
       * kept as written. One of our own names left open is still the tagger's
       * defect and still reported below; a name we do not know that *is* closed
       * is a label somebody declared, and is still read as one.
       */
      if (node.unclosed && !getTagSpec(node.name)) {
        text += node.raw
        walk(node.children)
        continue
      }
...
function fromConll(source: string): Draft {
  const rows = source
    .split('\n')
    /* told apart before the trailing whitespace goes: a `#` token with empty
       columns is `#\t\t`, and trimmed it would look like a bare comment mark */
...
function finish(draft: Draft): Annotated {
  ...
  const ordered = [...draft.spans].sort(outerFirst)
