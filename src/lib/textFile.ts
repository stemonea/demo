...
export async function readTextFile(
  ...
  if (!file) return fail('No file was given.')

  const pattern = new RegExp(`\\.(${extensions.join('|')})$`, 'i')
  if (!pattern.test(file.name)) {
    return fail(`Only ${spell(extensions)} files are accepted \u2014 \u201c${file.name}\u201d is not one.`)
  }
  if (file.size > MAX_TEXT_BYTES) {
    return fail(`That file is ${Math.round(file.size / 1024)} KB; the limit is 400 KB.`)
  }
  ...
  try {
    ...
    return fail(`Could not read the file: ${(cause as Error).message}`)
  }
}
