...
/** how many earlier states Undo can go back through */
const UNDO_LIMIT = 100

/** The annotation as the sheet last saw it, and every state it has been in before. */
interface History {
  /** the text the states are offsets into; a new text starts a new history */
  text: string
  spans: ManualSpan[]
  /** earlier states, newest last */
  past: ManualSpan[][]
  /** the state an undo has just asked the host for, so its arrival is not recorded as a change */
  restoring: ManualSpan[] | null
}

/**
 * Whether the sheet is the one in front of the reader, rather than mounted on a
 * slide out of sight.
 *
 * On screen is not enough inside a deck: while the deck travels from one slide
 * to the next, both are partly visible, and a key pressed then belongs to the
 * slide the deck already counts as current - the one nearer the start of the
 * track, by the same half-way rule `SlideDeck` rounds its index with.
 */
function inView(element: HTMLElement | null): boolean {
  if (!element) return false
  const box = element.getBoundingClientRect()
  const onScreen = box.width > 0 && box.right > 0 && box.bottom > 0 && box.left < window.innerWidth && box.top < window.innerHeight
  if (!onScreen) return false

  const slide = element.closest('.deck__slide')
  const track = slide?.parentElement
  if (!slide || !track) return true
  const offset = slide.getBoundingClientRect().left - track.getBoundingClientRect().left
  return Math.abs(offset) < slide.clientWidth / 2
}
...
export default function SpanCorrector({
  ...
  /*
   * The annotation as this sheet last saw it, and every state before it: what
   * Undo and ⌫ go back to.
   *
   * Undo used to take off whichever span was last in the list. That is the last
   * thing done only when the last thing done was adding one - after a boundary
   * was moved, a class changed or a span removed, it deleted a span that had
   * nothing to do with it. So every change is kept, whoever made it: this
   * sheet, the list beside it, a reset in the host. A change is noticed the way
   * a new text is below, by the spans handed in not being the ones last seen.
   */
  const [history, setHistory] = useState<History>(() => ({ text, spans, past: [], restoring: null }))
  ...
  /* The text is the one thing a selection cannot survive: a new turn, a new
     ...
     points into text that is gone - and the history goes with it, because the
     states in it are offsets into that text. */
  if (history.text !== text) {
    setHistory({ text, spans, past: [], restoring: null })
    setSelection(null)
    setMenu(null)
  } else if (history.spans !== spans) {
    /* the state an undo asked for is arriving, and is not one more to undo */
    const past = spans === history.restoring ? history.past : [...history.past, history.spans].slice(-UNDO_LIMIT)
    setHistory({ text, spans, past, restoring: null })
  }
  ...
  /** Back to the annotation as it was before the last change, whatever that change was. */
  const undo = useCallback(() => {
    const previous = history.past.at(-1)
    if (!previous) return
    setHistory((was) => ({ ...was, past: was.past.slice(0, -1), restoring: previous }))
    setMenu(null)
    say(null)
    onChange(previous)
  }, [history.past, onChange, say])
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      ...
      /* heard on the window, so the sheet has to be the one in front of the
         reader: a deck keeps every slide mounted, and a Backspace pressed on
         another of them used to undo work on this sheet out of sight */
  ...
  }, [assign, clearSelection, editing, history.past.length, nudge, onActiveSpan, onEditing, palette, selection, spans, undo])
  ...
  const placed: Placed[] = useMemo(() => {
    ...
    /* on the very same words the argument component is the container, as it
       is in the markup - `<premise><date>2022</date></premise>` */
    return [...withBounds].sort(
      (a, b) => a.first - b.first || b.last - b.first - (a.last - a.first) || outerFirst(a, b),
    )
  }, [spans, tokens])
  ...
  function openSpanMenu(span: Placed, event: React.MouseEvent) {
    ...
    ].sort((a, b) => a.last - a.first - (b.last - b.first) || outerFirst(b, a))
